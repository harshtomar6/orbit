...
import { initialConnectionSelection, reconciledConnectionSelection, savedConnectionItems, shouldShowLocalOnboarding } from "./lib/connection-selection";
...
const defaultPort = (kind: DatabaseKind) => kind === "postgres" ? 5432 : kind === "mysql" || kind === "mariadb" ? 3306 : 27017;
const defaultConnectionString = (kind: DatabaseKind) => kind === "mongodb" ? "mongodb://localhost:27017" : "";
const databaseDisplayName = (kind: DatabaseKind) => kind === "postgres" ? "PostgreSQL" : kind === "mysql" ? "MySQL" : kind === "mariadb" ? "MariaDB" : "MongoDB";
...
const readRecentExploreTargets = (mode: DatabaseTransportMode): ExploreTabTarget[] => { try { const value: unknown = JSON.parse(localStorage.getItem(`orbit.exploreRecent.${mode}`) ?? "[]"); if (!Array.isArray(value)) return []; return value.filter((item): item is ExploreTabTarget => Boolean(item && typeof item === "object" && "connectionId" in item && typeof item.connectionId === "string" && "connectionName" in item && typeof item.connectionName === "string" && "databaseKind" in item && ["mongodb", "postgres", "mysql", "mariadb"].includes(String(item.databaseKind)) && "namespace" in item && typeof item.namespace === "string" && "object" in item && typeof item.object === "string" && "objectKind" in item && ["collection", "table", "view"].includes(String(item.objectKind)))).filter((item) => item.databaseKind !== "mongodb" || (!isMongoSystemNamespace(item.namespace) && !isMongoSystemCollection(item.object))).slice(0, 10); } catch { return []; } };
...
const reconcileConnections = (current: DatabaseConnection[], next: DatabaseConnection[]) => next.map((item) => {
...
const emptyConnection = (): ConnectionInput => ({ name: "", kind: "mongodb", environment: "development", host: "localhost", port: 27017, database: "", username: "", password: "", tls: true, sslMode: "require", authSource: "admin", directConnection: false, connectTimeoutMs: 10_000 });
type ConnectionMethod = "uri" | "fields";
...
function ConnectionAccessFields({ form, method, connectionString, onFormChange, onMethodChange, onConnectionStringChange }: { form: ConnectionInput; method: ConnectionMethod; connectionString: string; onFormChange: (form: ConnectionInput) => void; onMethodChange: (method: ConnectionMethod) => void; onConnectionStringChange: (value: string) => void }) {
  const mongo = form.kind === "mongodb";
  const mysqlCompatible = form.kind === "mysql" || form.kind === "mariadb";
  const placeholder = mongo ? "mongodb+srv://user:password@cluster.example.net" : form.kind === "postgres" ? "postgresql://user:password@host:5432/database" : "mysql://user:password@host:3306/database";
  return <>
    <div className="connection-method" role="group" aria-label="Connection method"><button type="button" className={method === "uri" ? "active" : ""} onClick={() => onMethodChange("uri")}><AppIcon name="copy" size={13} /> Connection string</button><button type="button" className={method === "fields" ? "active" : ""} onClick={() => onMethodChange("fields")}><AppIcon name="database" size={13} /> Host & credentials</button></div>
    {method === "uri" ? <label>Connection string<input required type="password" autoComplete="off" spellCheck={false} value={connectionString} placeholder={placeholder} onChange={(event) => onConnectionStringChange(event.target.value)} /><small>Paste the complete URL from your provider. Orbit preserves provider-specific parameters.</small></label> : <>
      <div className="form-row host-port-row"><label>Host<input required value={form.host} placeholder="localhost" onChange={(event) => onFormChange({ ...form, host: event.target.value })} /></label><label>Port<input required type="number" min={1} max={65535} value={form.port} onChange={(event) => onFormChange({ ...form, port: Number(event.target.value) })} /></label></div>
      <label>{mongo ? "Default database (optional)" : "Database"}<input required={!mongo} value={form.database} placeholder={mongo ? "Leave empty to browse all databases" : "app"} onChange={(event) => onFormChange({ ...form, database: event.target.value })} /></label>
      <div className="form-row"><label>Username<input required={!mongo} autoComplete="username" value={form.username} placeholder={mongo ? "Optional" : "reader"} onChange={(event) => onFormChange({ ...form, username: event.target.value })} /></label><label>Password<input type="password" autoComplete="new-password" value={form.password} placeholder={mongo ? "Optional" : "Password"} onChange={(event) => onFormChange({ ...form, password: event.target.value })} /></label></div>
      <details className="connection-advanced"><summary><span>Advanced options</span><small>TLS, timeout{mongo ? ", authentication and replica set" : " and SSL mode"}</small><AppIcon name="chevron" size={13} /></summary><div>
        {mongo ? <><div className="form-row"><label>Authentication database<input value={form.authSource ?? ""} placeholder="admin" onChange={(event) => onFormChange({ ...form, authSource: event.target.value })} /></label><label>Replica set<input value={form.replicaSet ?? ""} placeholder="Optional" onChange={(event) => onFormChange({ ...form, replicaSet: event.target.value })} /></label></div><label className="checkbox"><input type="checkbox" checked={form.directConnection ?? false} onChange={(event) => onFormChange({ ...form, directConnection: event.target.checked })} /> Connect directly to this host</label></> : <label>SSL mode<select value={form.sslMode ?? "require"} onChange={(event) => onFormChange({ ...form, sslMode: event.target.value as ConnectionInput["sslMode"], tls: event.target.value !== "disable" })}>{mysqlCompatible ? <><option value="disable">Disabled</option><option value="prefer">Preferred</option><option value="require">Required</option><option value="verify-ca">Verify certificate authority</option><option value="verify-full">Verify identity</option></> : <><option value="disable">Disable</option><option value="prefer">Prefer</option><option value="require">Require</option><option value="verify-ca">Verify CA</option><option value="verify-full">Verify full</option></>}</select></label>}
        {mongo && <label className="checkbox"><input type="checkbox" checked={form.tls} onChange={(event) => onFormChange({ ...form, tls: event.target.checked })} /> Require TLS</label>}
        <label>Connection timeout<input type="number" min={1} max={120} value={Math.round((form.connectTimeoutMs ?? 10_000) / 1_000)} onChange={(event) => onFormChange({ ...form, connectTimeoutMs: Number(event.target.value) * 1_000 })} /><small>Seconds before a connection attempt is cancelled.</small></label>
      </div></details>
    </>}
  </>;
}
...
export function App() {
  ...
  const [enteredWithDiscoveredConnection, setEnteredWithDiscoveredConnection] = useState(false);
  const [editing, setEditing] = useState<DatabaseConnection>(); const [saving, setSaving] = useState(false); const [managerError, setManagerError] = useState(""); const [testingId, setTestingId] = useState(""); const [discovering, setDiscovering] = useState(false);
  const [pendingRemoval, setPendingRemoval] = useState<DatabaseConnection>(); const [removingId, setRemovingId] = useState("");
  ...
  const savedConnections = savedConnectionItems(connections);
  const dockerConnectionGroups = useMemo(() => {
    const groups = new Map<string, DatabaseConnection[]>();
    for (const item of connections.filter((candidate) => candidate.ephemeral && candidate.source?.kind === "docker")) {
      const project = item.source?.project ?? "Docker";
  ...
  const reloadConnections = useCallback(async () => {
    const items = await dbApi.connections();
    setConnections((current) => reconcileConnections(current, items));
    setConnectionId((current) => {
      const next = reconciledConnectionSelection(items, current);
      if (next === current) return current;
      connectionSelection.current[transportMode] = next;
      if (next) localStorage.setItem(`orbit.connection.${transportMode}`, next); else localStorage.removeItem(`orbit.connection.${transportMode}`);
      setObjectName(""); setObjects([]); setNamespaces([]); setResult(undefined);
      return next;
    });
    return items;
  }, [dbApi, transportMode]);
  ...
  useEffect(() => {
    ...
    dbApi.connections().then((items) => {
      ...
      const id = initialConnectionSelection(items, preferred);
  ...
  useEffect(() => {
    if (runtime !== "desktop" || transportMode !== "local") return;
    const interval = window.setInterval(() => { void reloadConnections().catch(() => undefined); }, 5_000);
    return () => window.clearInterval(interval);
  }, [runtime, transportMode, reloadConnections]);
  ...
  function openCreate() { setEditing(undefined); setConnectionForm(emptyConnection()); setConnectionString(defaultConnectionString("mongodb")); setConnectionMethod("uri"); setManagerError(""); setFormOpen(true); }
  ...
  async function saveConnection(event: FormEvent) { event.preventDefault(); setSaving(true); setManagerError(""); try { const input = connectionMethod === "uri" ? { ...connectionForm, connectionString: connectionString.trim() } : { ...connectionForm, connectionString: undefined }; const saved = editing ? await dbApi.updateConnection(editing.id, { name: connectionForm.name, environment: connectionForm.environment }) : await dbApi.createConnection(input); await reloadConnections(); if (!editing) selectConnection(saved.id); setFormOpen(false); } catch (reason) { setManagerError(reason instanceof Error ? reason.message : String(reason)); } finally { setSaving(false); } }
  ...
  async function pinManagedConnection(item: DatabaseConnection) { if (!dbApi.pinConnection) return; setTestingId(item.id); setManagerError(""); try { const pinnedConnection = await dbApi.pinConnection(item.id); await reloadConnections(); if (connectionId === item.id) selectConnection(pinnedConnection.id); } catch (reason) { setManagerError(reason instanceof Error ? reason.message : String(reason)); } finally { setTestingId(""); } }
  async function scanDockerConnections() { setDiscovering(true); setManagerError(""); try { const items = dbApi.scanDockerConnections ? await dbApi.scanDockerConnections() : await dbApi.connections(); setConnections((current) => reconcileConnections(current, items)); if (!items.some((item) => item.id === connectionId)) selectConnection(reconciledConnectionSelection(items, "")); } catch (reason) { setManagerError(reason instanceof Error ? reason.message : String(reason)); } finally { setDiscovering(false); } }
  async function removeManagedConnection(item: DatabaseConnection) { setRemovingId(item.id); setManagerError(""); try { await dbApi.removeConnection(item.id); const items = await reloadConnections(); if (connectionId === item.id) selectConnection(reconciledConnectionSelection(items, "")); setPendingRemoval(undefined); } catch (reason) { setManagerError(reason instanceof Error ? reason.message : String(reason)); } finally { setRemovingId(""); } }
  ...
  const renderConnectionOption = (item: DatabaseConnection) => <button className={item.id === connectionId ? "active" : ""} key={item.id} onClick={() => selectConnection(item.id)}><span className={`db-icon ${item.kind}`}>{icon(item.kind)}</span><span><strong>{item.name}</strong><small>{item.ephemeral ? `${item.database} · ${item.source?.health ?? "running"}` : `${item.environment} · ${item.database} · ${item.latencyMs ?? "—"} ms`}</small></span>{item.id === connectionId && <b>✓</b>}</button>;
  ...
  const showOnboarding = (transportMode === "local" && shouldShowLocalOnboarding(connections, loading, Boolean(error), connectionId, enteredWithDiscoveredConnection)) || (import.meta.env.DEV && params.get("preview") === "onboarding");
  if (showOnboarding) return <div className={`onboarding-shell ${runtime}`}>
    ...
    <main className="onboarding-main">
      ...
      <section className="onboarding-card">
        {onboardingStep === 1 ? <><header><small>STEP 1 OF 2</small><h2>What are you connecting?</h2><p>Open a running local database or configure a direct, read-only connection.</p></header><div className="database-choices">{dockerConnectionGroups.length > 0 && <><section className="onboarding-local-connections"><header><div><strong>Running locally</strong><small>Detected from Docker on this device</small></div><b>LIVE</b></header><div>{dockerConnectionGroups.map(([project, items]) => <section className="onboarding-local-group" key={project}><small>{project}</small>{items.map((item) => <article key={item.id}><span className={`db-icon ${item.kind}`}>{icon(item.kind)}</span><div><strong>{item.name}</strong><small>{databaseDisplayName(item.kind)} · {item.database} · {item.source?.health ?? "running"}</small></div><button type="button" onClick={() => selectConnection(item.id)}>Explore <AppIcon name="chevron" size={12} /></button></article>)}</section>)}</div></section><div className="onboarding-choice-divider"><span>Or connect manually</span></div></>}{([{"kind":"mongodb","name":"MongoDB","description":"Atlas, replica sets, and local MongoDB."},{"kind":"postgres","name":"PostgreSQL","description":"Hosted or local PostgreSQL databases."},{"kind":"mysql","name":"MySQL","description":"Hosted or local MySQL databases."},{"kind":"mariadb","name":"MariaDB","description":"MariaDB servers using the MySQL protocol."}] as const).map((option) => <button className={connectionForm.kind === option.kind ? "selected" : ""} key={option.kind} onClick={() => { setConnectionForm({ ...emptyConnection(), kind: option.kind, port: defaultPort(option.kind) }); setConnectionString(defaultConnectionString(option.kind)); setConnectionMethod("uri"); }}><span className={`db-icon ${option.kind}`}>{icon(option.kind)}</span><div><strong>{option.name}</strong><p>{option.description}</p></div><i>{connectionForm.kind === option.kind ? "✓" : ""}</i></button>)}</div><footer><span>You can add more connections later.</span><button className="primary" onClick={() => setOnboardingStep(2)}>Continue <AppIcon name="chevron" size={14} /></button></footer></> : <form onSubmit={saveConnection}><header><small>STEP 2 OF 2</small><h2>Connect to {databaseDisplayName(connectionForm.kind)}</h2><p>Orbit tests the connection before storing it on this device.</p></header><div className="onboarding-fields"><div className="form-row"><label>Connection name<input autoFocus required value={connectionForm.name} placeholder="Production analytics" onChange={(event) => setConnectionForm({ ...connectionForm, name: event.target.value })} /></label><label>Environment<select value={connectionForm.environment} onChange={(event) => setConnectionForm({ ...connectionForm, environment: event.target.value as ConnectionEnvironment })}><option value="development">Development</option><option value="staging">Staging</option><option value="production">Production</option></select></label></div><ConnectionAccessFields form={connectionForm} method={connectionMethod} connectionString={connectionString} onFormChange={setConnectionForm} onMethodChange={setConnectionMethod} onConnectionStringChange={setConnectionString} />{managerError && <div className="form-error">{managerError}</div>}</div><footer><button type="button" onClick={() => { setManagerError(""); setOnboardingStep(1); }}>Back</button><button className="primary" disabled={saving}>{saving ? <><span className="button-spinner" /> Testing connection…</> : <>Test & connect <AppIcon name="chevron" size={14} /></>}</button></footer></form>}
      </section>
      <aside className="onboarding-guide">
        ...
        <div>
          ...
          <p><AppIcon name="copy" size={16} /><span>A provider connection string, or the host, port, database, and credentials.</span></p>
  ...
  return <div className={`orbit-shell ${runtime}`}>
    <header className="orbit-topbar">
      ...
      <div className="connection-wrap" ref={connectionMenuRef}><button className="connection-button" onClick={() => setMenu(!menu)} aria-expanded={menu}>{connection ? <><span className={`db-icon ${connection.kind}`}>{icon(connection.kind)}</span><span className="connection-copy"><strong>{connection.name}</strong><small>{connection.ephemeral ? `${connection.source?.project ?? "Docker"} · Auto-detected` : `${connection.database} · ${transportMode === "local" ? "Direct" : "Gateway"}`}</small></span><span className={`mode-dot ${connection.status === "unavailable" ? "unavailable" : transportMode}`} /><AppIcon name="chevron" size={13} /></> : <><AppIcon name="database" /><strong>No connection</strong><AppIcon name="chevron" size={13} /></>}</button>{menu && <div className="connection-menu">{savedConnections.length > 0 && <><label>{transportMode === "local" ? "Saved connections" : "Workspace connections"}</label>{savedConnections.map(renderConnectionOption)}</>}{dockerConnectionGroups.length > 0 && <><label className="docker-discovery-label"><span>Development environments</span><b>LIVE</b></label>{dockerConnectionGroups.map(([project, items]) => <section className="connection-project-group" key={project}><header><span>{project}</span><small>{items.length} database{items.length === 1 ? "" : "s"}</small></header>{items.map(renderConnectionOption)}</section>)}</>}<button className="manage" onClick={() => { setMenu(false); setManaging(true); }}><AppIcon name="plus" /> Add or manage connections</button><button className="appearance" onClick={() => { setTheme((current) => current === "dark" ? "light" : "dark"); setMenu(false); }}><AppIcon name={theme === "dark" ? "sun" : "moon"} /><span><strong>Appearance</strong><small>{theme === "dark" ? "Dark" : "Light"} mode</small></span></button></div>}</div>
    </header>
    ...
    {managing && <div className="modal-backdrop"><section className="connection-manager" aria-label="Connection manager">
      <header><div><small>{transportMode === "local" ? "LOCAL CONNECTIONS" : "WORKSPACE CONNECTIONS"}</small><h2>{formOpen ? editing ? "Edit connection" : "Add connection" : "Connections"}</h2></div><button onClick={() => { if (formOpen) setFormOpen(false); else { setManaging(false); setPendingRemoval(undefined); } }}>×</button></header>
      {managerError && <div className="form-error">{managerError}</div>}
      {formOpen ? <form onSubmit={saveConnection}>
        {!editing && <label className="database-kind-field">Database<select autoFocus value={connectionForm.kind} onChange={(event) => { const kind = event.target.value as DatabaseKind; setConnectionForm({ ...emptyConnection(), kind, port: defaultPort(kind) }); setConnectionString(defaultConnectionString(kind)); setConnectionMethod("uri"); }}><option value="mongodb">MongoDB</option><option value="postgres">PostgreSQL</option><option value="mysql">MySQL</option><option value="mariadb">MariaDB</option></select><small>Choose the database engine before entering connection details.</small></label>}
        ...
        {!editing && <ConnectionAccessFields form={connectionForm} method={connectionMethod} connectionString={connectionString} onFormChange={setConnectionForm} onMethodChange={setConnectionMethod} onConnectionStringChange={setConnectionString} />}
      ...
      </form> : <>{dockerConnectionGroups.length > 0 && <div className="docker-discovery-note"><span className="docker-mark">◆</span><div><strong>Local development databases detected</strong><small>Orbit reads Docker metadata in memory. Pin a connection to save its credentials in your keychain.</small></div></div>}<div className="managed-list">{connections.map((item) => <article className={item.ephemeral ? "ephemeral" : ""} key={item.id}><span className={`db-icon ${item.kind}`}>{icon(item.kind)}</span><div><strong>{item.name}{item.ephemeral && <span className="auto-detected-badge">AUTO</span>}</strong><small>{item.ephemeral ? `${item.source?.project ?? "Docker"} · ${item.source?.image ?? item.kind}` : `${databaseDisplayName(item.kind)} · ${item.environment} · ${item.database}`}</small><small>{item.ephemeral ? `${item.source?.health ?? item.status} · exposed to this Mac` : `${item.status} · ${item.latencyMs ?? "—"} ms${item.lastSchemaRefresh ? ` · refreshed ${new Date(item.lastSchemaRefresh).toLocaleString()}` : ""}`}</small></div><button disabled={testingId === item.id || removingId === item.id} onClick={() => testManagedConnection(item.id)}>{testingId === item.id ? "Testing…" : "Test"}</button>{item.ephemeral ? <button className="primary compact" disabled={testingId === item.id} onClick={() => pinManagedConnection(item)}>Pin</button> : <><button disabled={item.demo || removingId === item.id} onClick={() => openEdit(item)}>Edit</button><button className="danger-button" disabled={item.demo || removingId === item.id} onClick={() => setPendingRemoval(item)}>{removingId === item.id ? "Removing…" : "Remove"}</button></>}</article>)}</div><footer>{runtime === "desktop" && transportMode === "local" && <button className="secondary" disabled={discovering} onClick={() => void scanDockerConnections()}><AppIcon name="refresh" size={13} /> {discovering ? "Scanning…" : "Scan Docker"}</button>}<button className="primary" onClick={openCreate}>＋ Add connection</button></footer></>}
    </section></div>}
    {pendingRemoval && <div className="modal-backdrop confirmation-backdrop"><section className="confirmation-dialog" role="alertdialog" aria-modal="true" aria-labelledby="remove-connection-title"><span className="confirmation-icon"><AppIcon name="database" size={18} /></span><h2 id="remove-connection-title">Remove {pendingRemoval.name}?</h2><p>Orbit will remove this connection and its saved credential from this device. This does not modify the database.</p><div className="confirmation-connection"><span className={`db-icon ${pendingRemoval.kind}`}>{icon(pendingRemoval.kind)}</span><div><strong>{pendingRemoval.name}</strong><small>{databaseDisplayName(pendingRemoval.kind)} · {pendingRemoval.database}</small></div></div><footer><button disabled={Boolean(removingId)} onClick={() => setPendingRemoval(undefined)}>Cancel</button><button className="danger-button primary-danger" disabled={Boolean(removingId)} onClick={() => void removeManagedConnection(pendingRemoval)}>{removingId ? "Removing…" : "Remove connection"}</button></footer></section></div>}
  </div>;
}
