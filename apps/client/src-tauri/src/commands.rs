...
async fn mongo_collection_objects(connection_id: &str, client: &mongodb::Client, database: &str) -> Result<Vec<DataObject>, String> {
    let collections = client.database(database).list_collection_names().authorized_collections(true).await.map_err(|error| format!("Could not list collections in {database}: {error}"))?;
