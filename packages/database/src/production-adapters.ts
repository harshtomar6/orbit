...
export const isMongoSystemNamespace = (namespace: string) => mongoSystemNamespaces.has(namespace.toLowerCase());
export const isMongoSystemCollection = (collection: string) => collection.toLowerCase().startsWith("system.");
...
export async function createMongoAdapter(connection: DatabaseConnection, connectionUri: string): Promise<DatabaseAdapter> {
  ...
  return { kind: "mongodb",
    ...
    async listNamespaces(options) { const result = await timed(client.db("admin").admin().listDatabases({ authorizedDatabases: true, nameOnly: true }), options); return result.databases.map((database) => database.name).filter((namespace) => !isMongoSystemNamespace(namespace)).sort(); },
    async listObjects(connectionId, options, namespace = connection.database) { const values = await timed(client.db(namespace).listCollections({}, { nameOnly: true }).toArray(), options); const visible = values.filter((value) => !isMongoSystemCollection(value.name)); const objects = await Promise.all(visible.map(async (value): Promise<DataObject> => { const estimatedRows = await timed(client.db(namespace).collection(value.name).estimatedDocumentCount(), { ...options, timeoutMs: Math.min(options.timeoutMs, 2_000) }).catch(() => undefined); return { connectionId, namespace, name: value.name, kind: "collection", ...(estimatedRows === undefined ? {} : { estimatedRows }) }; })); return objects.sort((left, right) => left.name.localeCompare(right.name)); },
    describeObject: describe,
