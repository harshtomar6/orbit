...
export async function createMongoAdapter(connection: DatabaseConnection, connectionUri: string): Promise<DatabaseAdapter> {
  ...
  return { kind: "mongodb",
    ...
    async listObjects(connectionId, options, namespace = connection.database) { const values = await timed(client.db(namespace).listCollections({}, { nameOnly: true }).toArray(), options); return values.filter((value) => !isMongoSystemCollection(value.name)).map((value): DataObject => ({ connectionId, namespace, name: value.name, kind: "collection" })).sort((left, right) => left.name.localeCompare(right.name)); },
    describeObject: describe,
